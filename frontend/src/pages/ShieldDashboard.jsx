...
function ShieldDashboard() {
  ...
  const fetchData = async () => {
    try {
      ...
      const [logsRes, rulesRes] = await Promise.all([
        axios.get(`${API}/api/logs?limit=50`),
  ...
  const addRule = async (e) => {
    ...
    try {
      await axios.post(`${API}/api/rules`, newRule);
    ...
    <div className="p-4">
      ...
      <div className="bg-gray-800 p-6 rounded-xl shadow-lg">
        <div className="flex items-center justify-between">
          ...
          <button onClick={fetchData} className="bg-teal-500 hover:bg-teal-600 text-white font-bold py-2 px-3 rounded-lg">
            Refresh
          </button>
        </div>
                ...
                <tr key={log.id || idx} className="hover:bg-gray-600 transition duration-150">
                  ...
                  <TableData>
                    <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                      ${log.threat_type === 'Prompt_Injection'
                        ? 'bg-yellow-100 text-yellow-800'
                        : log.threat_type === 'Hallucination'
      ...
      <div className="bg-gray-800 p-6 rounded-xl shadow-lg mt-8">
        ...
        <div className="mt-4">
          ...
          <ul className="space-y-1">
            {rules.map((r, idx) => (
              <li key={r.id || idx} className="text-gray-300 text-sm">
                • [{r.type}] {r.rule_name}: <span className="font-mono">{r.value}</span>
              </li>
            ))}
          </ul>
        </div>
